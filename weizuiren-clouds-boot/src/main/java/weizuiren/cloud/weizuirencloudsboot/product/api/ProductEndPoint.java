package weizuiren.cloud.weizuirencloudsboot.product.api;
...
import weizuiren.cloud.weizuirencloudsboot.product.dto.ProductCommentDto;
import weizuiren.cloud.weizuirencloudsboot.product.dto.ProductDto;
...
public class ProductEndPoint extends BaseEndPoint {
    ...
    public DataResponse<List<ProductCommentDto>> comments(@PathVariable Long id) {
        ...
        list = comments.stream().map((comment) -> {
            ...
            dto.setUserDto(new UserDto(this.userService.load(comment.getAuthorId())));
            return dto;
